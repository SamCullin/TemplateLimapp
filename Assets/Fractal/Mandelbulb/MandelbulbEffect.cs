...
public class MandelbulbEffect : MonoBehaviour {
	...
	void Update () {
        ...
        if (Input.GetAxis("Mouse ScrollWheel") > 0 || Input.GetKey(KeyCode.Q))
        {
            ...
            c.transform.position += c.transform.forward * moveSpeed * Time.deltaTime;
        }
        ...
        c.transform.position += c.transform.right * Input.GetAxis("Horizontal") * moveSpeed * Time.deltaTime;
        c.transform.position += c.transform.up * Input.GetAxis("Vertical") * moveSpeed * Time.deltaTime; // DX uses 0,0 in top left    
